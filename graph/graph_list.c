...
GRAPHlist graph_list_init()   {
    ...
    && (grafo_list->vrtx = (struct GrafoVertice **)calloc(MAX_graph, sizeof(struct GrafoVertice *)))    //array dei puntatori ai vertici del Grafo (inizializzati a NULL)
    && (grafo_list->heights = (struct Lista **)calloc(MAX_graph, sizeof(struct Lista *))))  { //array delle altezze registrate (inizializzati a NULL)
        grafo_list->n_vrtx = 0;
        grafo_list->idx_max = 0;   //in questa posizione verrà registrato e aggiornato l'ultimo indice utilizzato dai vertici del grafo
    } else {
        printf("[MEM] ATTENZIONE: Problema di allocazione ");
        if(!grafo_list)
            printf("GRAPHlist ");
        else if(!(grafo_list->vrtx))
            printf("GRAPHvrtx * ");
        else if(!(grafo_list->heights))
            printf("LIST * ");
...
void graph_list_insVertex(GRAPHlist grafo_list, int idx, int height, int dup) {     //se il peso è = 0, trattasi di inserimento manuale e non generazione randomica
    if(idx == -1)    {   //se non è stato indicato manualmente l'indice del vertice
        do  {
            idx++;
        }while(!(grafo_list->vrtx[idx]))  //cerco il primo vertice disponibile 
    }
    printf("DEBUG insVertex: %d\n", idx);
    if(idx > grafo_list->idx_max)    //se ho superato il margine massimo, aggiorno il suo contatore
        grafo_list->idx_max = idx; 
    if((grafo_list->vrtx[idx] = (struct GrafoVertice *)malloc(sizeof(struct GrafoVertice))))  {  //assegno un puntatore della Lista di adiacenza, pronta ad aggiungere archi ai vertici adiacenti
        if(height == -1)  { //se non è stato indicato manualmente l'altezza
            if(!dup)    {   //se non è possibile inserire nodi nella stessa altezza
                while(!(grafo_list->heights[height = random_num(0,255)]))  //cerco un'altezza casuale disponibile
                    ;
            } else  {//se posso inserire duplicati, va bene qualsiasi valore
                if(grafo_list->heights[height = random_num(0,255)])  {   //verrà scelta un altezza randomicamente; se esiste già un nodo che ha la stessa altezza...
                    int weight;
                    graph_list_insArc(grafo_list, idx, grafo_list->heights[height]->idx_vrtx_dst, (weight = random_num(0,255)))   //...applico subito questo nuovo nodo all'adiacente del vertice posto alla stessa altezza
        ...
        grafo_list->vrtx[idx]->height = height;                 //assegno l'altezza al vertice creato
        grafo_list->heights[height] = list_insertHead(grafo_list->heights[height], idx, -1);  //inserisco un elemento nella lista delle altezze con l'identificativo del nodo associato     
        grafo_list->n_vertex += 1;                              //aggiorno la grandezza del grafo
        printf("Vertice %d correttamente inserito nel Grafo in altezza %d\n", idx, grafo_list->vrtx[idx]->height);        
    } else  {
...
void graph_list_insArc(GRAPHlist grafo_list, int idx_src, int idx_dst, int weight) {
    ...
    if(idx_dst == -1)    {   //se la destinazione è da specificare
        ...
        printf("%d: ", idx_src);
        n_edge = list_visit(grafo_list->vrtx[idx_src]->adj, adj_slave);    //stampo e conto i vertici adiacenti, controllando la disponibilità di vertici collegabili
        printf("\n\n");
        if(n_edge < grafo_list->n_vrtx)   {
            printf("Vertici disponibili per %d:\n", idx_src);
            grafo_list_print(adj_slave, grafo_list->n_vertex, 0); //mostro i restanti vertici disponibili; con '0' al secondo parametro ('int showlist') non mostro le Liste di Adiacenza di questi vertici
            do  {
                ...
                if(!grafo_list->vrtx[idx_dst]->adj)
            ...
            }while(!grafo_list->vrtx[idx_dst]->adj || !adj_slave[idx_dst]);
    ...
    list_insertHead(grafo_list->vrtx[idx_src]->adj, idx_dst, weight); //inserimento in testo nella Lista di Adiacenza di idx_src
    list_insertHead(grafo_list->vrtx[idx_dst]->adj, idx_src, weight); //inserimento in testo nella Lista di Adiacenza di idx_dst (grafo non orientato)
    printf("Arco inserito in Lista fra i vertici %d <-> %d di peso [%d]\n", idx_src, idx_dst, grafo_list->vrtx[idx_src]->adj->weight);
}
...
void graph_list_deleteGraph(GRAPHlist grafo_list) {
    ...
    for(idx=0;idx<grafo_list->idx_max;idx++){
        if(grafo_list->vrtx[idx] && grafo_list->vrtx[idx]->adj)  {
            grafo_list->vrtx[idx]->adj = list_delete(grafo_list->vrtx[idx]->adj); //elimino tutta la Lista puntata ricorsivamente
            grafo_list->heights[grafo_list->vrtx[idx]->height] = list_delKey(grafo_list->heights[grafo_list->vrtx[idx]->height], idx); //elimino la corrispondente altezza nell'array delle altezze
	        free(grafo_list->vrtx[idx]);   //dealloco il puntatore del vertice
        }
    }
	grafo_list->n_vrtx = 0;
    grafo_list->idx_max = 0;
}
...
void graph_list_delVertex(GRAPHlist grafo_list, int idx_del)  {
    grafo_list->vrtx[idx_del]->adj = graph_list_delVertex_undirectedArcs(grafo_list, grafo_list->vrtx[idx_del]->adj, idx_del);  //eliminazione degli archi entranti al vertice e la sua Lista di Adiacenza
    grafo_list->heights[grafo_list->vrtx[idx_del]->height] = list_delKey(grafo_list->heights[grafo_list->vrtx[idx_del]->height], idx_del); //elimino la corrispondente altezza nell'array delle altezze
    free(grafo_list->vrtx[idx_del]);    //dealloco il vertice
    grafo_list->vrtx[idx_del] = NULL;   //rendo disponibile lo spazio nel caso di inserimento di un nuovo vertice
    ...
    if(idx_del == grafo_list->idx_max) {//se il vertice da eliminare è proprio l'ultimo, aggiorno anche il valore del margine massimo dell'array
        do{ //cerco il minimo vertice disponibile
            grafo_list->idx_max -= 1;   //decremento fin quando non trovo un vertice presente
        }while(grafo_list->idx_max > 0 && !(grafo_list->vrtx[grafo_list->idx_max]))
    }
    grafo_list->n_vertex -= 1;  //IMPORTANTE: in caso di ultimo elemento eliminato, cancellare il grafo
}
...
LIST graph_list_delVertex_undirectedArcs(GRAPHlist grafo_list, LIST L_curr, int idx_del) {
    if(L_curr)  {
        L_curr->next = graph_list_delVertex_undirectedArcs(grafo_list, L_curr->next, idx_del);
        graph_list->vrtx[L_curr->idx_vrtx_dst]->adj = list_delKey(graph_list->vrtx[L_curr->idx_vrtx_dst]->adj, idx_del);    //eliminazione dell'arco entrante
...
void graph_list_delArc(GRAPHlist grafo_list, int idx_src) {
    if(graph_list->vrtx[idx_src]->adj)    {
        int idx_del;
        LIST *adj_slave = graph_list_adj_mirror(grafo_list);

        printf("%d: ", idx_src);
        list_visit((grafo_list->vrtx[idx_src]->adj), adj_slave);    //stampo e conto i vertici adiacenti, controllando la disponibilità di vertici collegabili
        printf("\n\n");
        do  {
        ...
        graph_list->vrtx[idx_src]->adj = list_delKey(graph_list->vrtx[idx_src]->adj, idx_del);    //eliminazione dell'arco uscente
        graph_list->vrtx[idx_src]->adj = list_delKey(graph_list->vrtx[idx_del]->adj, idx_src);    //eliminazione dell'arco entrante
    } else
        printf("ATTENZIONE: il vertice scelto non ha archi uscenti\n\n");
}

//Visualizzazione del percorso fra due vertici in BFS o DFS
void graph_list_path(GRAPHlist grafo_list, int idx_src, int idx_dst, int mode)  {   //mode == 0 -> BFS; mode == 1 -> DFS
    if(idx_src != idx_dst)  {
        int *pred = NULL;
        if(!mode)   //BFS
            pred = graph_list_BFS(grafo_list, idx_src);
        else        //DFS
            pred = graph_list_DFS(grafo_list, idx_src);
...
int *graph_list_BFS(GRAPHlist grafo_list, int idx_src)  {
    int idx;
    int *pred = (int *)calloc(grafo_list->idx_max, sizeof(int));    //creo l'array degli indici dei predecessori
...
void graph_list_DFS_visit(GRAPHlist grafo_list, int idx_curr, int *pred, char *color)    {
    ...
    while(adj_curr)    {    //ciclo fin quando non svuoto la coda
        if(color[adj_curr->idx_vrtx_dst] == 'w')  { //se BIANCO
            ...
            graph_list_DFS_visit(grafo_list, adj_curr->idx_vrtx_dst, pred, color);    //visito il nodo appena incontrato
        }
        adj_curr = adj_curr->next;  //passo al prossimo vertice adiacente
    }
...
void grafo_list_print(GRAPHlist grafo_list, int showlist)   {
    int idx;
    for(idx=0;idx<grafo_list->idx_max;idx++) {
        if(grafo_list->vrtx[idx])   { //se esiste il vertice nell'array
            printf("%d", idx);
            if(showlist)    //si può richiedere dal parametro 'showlist' passando '1' per mostrare la Lista di Adiacenza del vertice attuale
                printf(":");
                list_visit(grafo_list->vrtx[idx]->adj, NULL); //passo la lista di adiacenza del vertice attuale
            printf("\n");
        }
    }
}
...
LIST *graph_list_adj_mirror(GRAPHlist grafo_list)    {
    int idx;
    LIST *adj_slave = NULL;
    if(adj_slave = (LIST *)calloc(grafo_list->idx_max, sizeof(LIST))) {     //array slave delle Liste di Adiacenza da riferire
        for(idx=0;idx<grafo_list->idx_max;idx++)
            adj_slave[idx] = grafo_list->vrtx[idx]->adj;    //copio i riferimenti delle Liste di Adiacenza di tutti i nodi (disponibili e non);
    }
