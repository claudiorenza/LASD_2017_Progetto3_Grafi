...
GRAPHlist graph_list_init(int idx_max)   {
    ...
    && (grafo_list->vrtx = (struct GrafoVertice **)calloc(MAX_graph, sizeof(struct GrafoVertice *)))
    && (grafo_list->heights = (struct Lista **)calloc(MAX_graph, sizeof(struct Lista *))))  { //array dei puntatori ai vertici del Grafo (inizializzati a NULL)
        grafo_list->n_vrtx = 0;
...
void graph_list_insVertex(GRAPHlist grafo_list, int idx, int height, int dup) {
    ...
    if((grafo_list->vrtx[idx] = (struct GrafoVertice *)malloc(sizeof(struct GrafoVertice))))  {  //assegno un puntatore della Lista di adiacenza, pronta ad aggiungere archi ai vertici adiacenti
        if(height == -1)  { //se non è stato indicato manualmente l'altezza e non dovranno esserci duplicati
            if(!dup)    {
                ...
                height = random_num(0,255);
        }  
        grafo_list->vrtx[idx]->height = height;                 //assegno l'altezza al vertice creato
        if(grafo_list->heights[height])     //se esiste già un nodo che ha la stessa altezza
            graph_list_insArc(grafo_list, idx, grafo_list->heights[height]->idx_vrtx_dst)   //IMPORTANTE: scegliere dalla lista delle altezze una delle sue destinazioni in maniera casuale
        list_insertHead(grafo_list->heights[height], idx, -1);  //inserisco un elemento nella lista delle altezze con l'identificativo del nodo associato

        
        grafo_list->n_vertex += 1;                              //indico la grandezza del grafo
...
GRAPHlist graph_list_deleteGraph(GRAPHlist grafo_list) {
    ...
    for(idx=0;idx<grafo_list->idx_max;idx++){
        if(grafo_list->vrtx[idx]->adj)
            grafo_list->vrtx[idx]->adj = graph_list_nodeFree(grafo_list->vrtx[idx]->adj);
    }
    return NULL;
}
...
void graph_list_delVertex(GRAPHlist grafo_list, int idx_del, int n_vrtx)  {
    int idx;
    for(idx=0;idx<=grafo_list->idx_max;idx++) {
        if(grafo_list->vrtx[idx]->adj && idx!=idx_del)
            list_delKey(grafo_list->vrtx[idx]->adj, idx_del); //CHECK: passaggio del riferimento
    ...
    if(idx_del == grafo_list->idx_max) {//se il vertice da eliminare è proprio l'ultimo, aggiorno anche il valore del margine massimo dell'array
        while(idx_del > 0 && !(grafo_list->vrtx[idx]->adj))    //cerco il minimo vertice disponibile
...
void graph_list_transpose(GRAPHlist grafo_list)    {
    ...
    for(idx=0;idx<=idx_max;idx++) {  //Inizializzazione con copia dei riferimenti alle Liste
        if(grafo_list->vrtx[idx]->adj)   {
            L_arr_vrtx[idx]->adj = ((GRAPHlist)grafo_list->vrtx[idx]->adj)->adj;     //memorizzo idx riferimenti nell'array delle Liste
            ((GRAPHlist)grafo_list->vrtx[idx]->adj)->adj = NULL;             //tolgo idx riferimenti nel grafo per gli inserimenti di 'list_trasp_recv'
        }
    }
    for(idx=0;idx<=idx_max;idx++) {  //Visita delle Liste e inserimento dei vertici nel grafo trasposto
        if(grafo_list->vrtx[idx]->adj)
            L_arr_vrtx[idx]->adj = list_trasp_recv(L_arr_vrtx[idx]->adj, (grafo_list), idx);    //N.B.: L_arr_vrtx[idx]->adj diventa NULL, quindi nessuna perdita di dati in deallocazione
    }
    free(L_arr_adj);
}
*/
...
void graph_list_check_cycle(GRAPHlist grafo_list)  {
    int idx=0;
    while(!grafo_list->vrtx[idx]->adj)   //vedo l'indice minimo del primo elemento presente attualmente nel grafo
        idx++;
...
int graph_list_DFS(GRAPHlist grafo_list, int idx_src)  {
    ...
    while(adj_curr) {
        if(color[adj_curr->idx_vrtx_dst] == 'w')   //se BIANCO
            check_cycle = graph_list_DFS_visit(grafo_list, adj_curr->idx_vrtx_dst, pred, color, check_cycle); //visito l'elemento della lista di adiacenza
        adj_curr = adj_curr->next;  //passo al prossimo vertice adiacente
    }
    free(color);
    return check_cycle;
...
int graph_list_DFS_visit(GRAPHlist grafo_list, int idx_curr, int *pred, char *color, int check_cycle)    {
    ...
    while(adj_curr)    {    //ciclo fin quando non svuoto la coda
        if(color[adj_curr->idx_vrtx_dst] == 'w')  { //se BIANCO
            ...
            check_cycle = graph_list_DFS_visit(grafo_list, adj_curr->idx_vrtx_dst, pred, color, check_cycle);    //visito il nodo appena incontrato
        } else if(!check_cycle && color[adj_curr->idx_vrtx_dst] == 'g')       //se mi ritrovo un GRIGIO, � un arco di ritorno (ciclo)
...
void graph_list_adj_print(LIST *grafo_list_adj, int idx_max, int showlist)   {
    ...
    for(idx=0;idx<=idx_max;idx++) {
        if(grafo_list_vrtx[idx]->adj)   { //se esiste il vertice nell'array delle liste
            printf("%d", idx);
            if(showlist)    //si può richiedere dal parametro 'showlist' passando '1' per mostrare la Lista di Adiacenza del vertice attuale
                printf(":");
                list_visit(*grafo_list_vrtx[idx]->adj, NULL); //passo la lista di adiacenza del vertice attuale
            printf("\n");
        }
    }
}
...
LIST *graph_list_adj_mirror(GRAPHlist grafo_list)    {
    ...
    if(adj_slave = (LIST *)calloc(grafo_list->idx_max, sizeof(LIST))) {
        for(idx=0;idx<=grafo_list->idx_max;idx++)
            adj_slave[idx] = grafo_list->vrtx[idx]->adj;    //copio i riferimenti di tutti i nodi (disponibili e non);
    }
